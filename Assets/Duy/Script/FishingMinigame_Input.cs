...
public class FishingMinigame_Input : MonoBehaviour
{
    ...
    private void Evaluate()
    {
        ...
        if (success)
        {
            if (resultText) resultText.text = successText;
            onCatch?.Invoke();
        }
        else
        {
            if (resultText) resultText.text = missText;
            onMiss?.Invoke();
        }
    }
    ...
    [SerializeField] private UnityEvent onThreeRetries; // assign DoSomething() in Inspector
    public void Retry()
    {
        retryCount++;
        speed += 1f;
        ...
        if (retryCount >= 3)
        {
            ...
            onThreeRetries?.Invoke(); // DoSomething()
        }
